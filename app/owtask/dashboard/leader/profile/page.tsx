...
export default function LeaderProfilePage() {
  const { currentUser, tasks, users } = useStore();
  const [form, setForm] = useState({
    ...
    bio: "Passionate team leader focused on delivering high-quality results.",
    location: "Jakarta, Indonesia",
  });
  const [saved, setSaved] = useState(false);

  const leaderTasks = tasks.filter((t) => t.leaderId === currentUser.id);
  const completedTasks = leaderTasks.filter((t) => t.status === "completed");
  const inProgressTasks = leaderTasks.filter((t) => t.status === "in_progress");
  const overdueTasks = leaderTasks.filter((t) => t.status === "overdue");
  const completionRate =
    leaderTasks.length > 0
      ? Math.round((completedTasks.length / leaderTasks.length) * 100)
      : 0;
  ...
  const activities = [
    ...
    {
      icon: FileText,
      text: "Created task 'API Authentication'",
      time: "Yesterday",
    ...
    {
      icon: Clock,
      text: "Updated deadline for 'Testing'",
      time: "2 days ago",
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4 -mt-12">
            ...
            <div className="flex-1 pt-2">
              <div className="flex items-center gap-2">
                ...
                <Badge className="gradient-primary text-white border-0 text-[10px] rounded-lg flex items-center gap-1">
