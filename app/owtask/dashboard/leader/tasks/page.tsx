...
import { Plus, Search, LayoutGrid, List, CheckSquare } from "lucide-react";
...
function LeaderTasksContent() {
  ...
  const filtered = leaderTasks.filter((t) => {
    ...
    return matchSearch && matchStatus && matchPriority && matchDivision;
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/owtask/dashboard/leader/tasks/new">
          <Button className="gradient-primary text-white shadow-lg shadow-violet-500/20 gap-2 rounded-xl">
      ...
      <Tabs defaultValue="kanban" className="w-full">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Select value={priorityFilter} onValueChange={setPriorityFilter}>
              ...
              <SelectContent className="bg-[#1a1a30] border-white/8">
                ...
                <SelectItem value="high" className="text-white/70">
                  High
                </SelectItem>
                <SelectItem value="medium" className="text-white/70">
                  Medium
                </SelectItem>
                <SelectItem value="low" className="text-white/70">
                  Low
                </SelectItem>
              </SelectContent>
            </Select>
        ...
        <TabsContent value="kanban">
          <KanbanBoard
            tasks={filtered}
            onTaskClick={(t) =>
              router.push(`/owtask/dashboard/leader/tasks/${t.id}`)
            }
          />
        </TabsContent>
            ...
            <div className="flex-1 overflow-y-auto min-h-0 p-5 theme-scrollbar">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {filtered.map((task, i) => (
                  ...
                  >
                    <Link href={`/owtask/dashboard/leader/tasks/${task.id}`}>
                      <Card className="bg-[#1e1e3a] border-white/6 hover:border-white/12 hover:shadow-lg hover:shadow-violet-500/5 transition-all duration-300 hover:-translate-y-1 h-full rounded-2xl">
                        <CardContent className="p-5">
                          <div className="flex items-start justify-between mb-3">
                            ...
                            <Badge
                              className={`${priorityColor(task.priority)} border-0 text-xs rounded-lg`}
                            >
                              {task.priority}
                            </Badge>
                          </div>
                            ...
                            <div className="flex -space-x-2">
                              {task.assigneeIds.slice(0, 3).map((id) => {
                                ...
                                return user ? (
                                  <div
                                    key={id}
                                    className="flex h-6 w-6 items-center justify-center rounded-full gradient-primary text-white text-[10px] font-medium border-2 border-[#1e1e3a]"
                                  >
                                    {user.name
                                      .split(" ")
                                      .map((n) => n[0])
